...
void main() {
  ...
  test('the current period is read in the app, not launched at the site', () {
    expect(home.contains('HoroscopesScreen'), isTrue,
    ...
    expect(RegExp(r"action:\s*'All twelve',[\s\S]{0,400}?onAction:\s*_readHerReadings")
            .hasMatch(home), isTrue,
        reason: '"All twelve" does not open the in-app reader, which sends '
  ...
  test('an older reading still goes to the site', () {
    ...
    // asked for.
    expect(home.contains('currentCovers('), isTrue,
        reason: 'nothing compares a reading against the current period, so '
            'either everything opens in the app or everything opens the site');
    expect(RegExp(r"_open\('/horoscopes/").hasMatch(home), isTrue,
  ...
  test('the link out of the app says where it goes', () {
    ...
    // as the app crashing.
    expect(screen.contains('shrutivtuber.com'), isTrue,
  ...
  test('an unwritten sign is not shown as a blank reading', () {
    ...
    // not written it yet.
    expect(service.contains("r['published']"), isTrue,
        reason: 'unpublished signs are not filtered out, so a sign she has not '
  ...
  test('unreachable and unwritten are told apart', () {
    ...
    expect(screen.contains('out of reach'), isTrue,
        reason: 'a network failure is shown as her having written nothing');
    expect(screen.contains('Nothing written'), isTrue,
